...
class SpatioTemporalEmbedding(nn.Module):
    ...
    def __init__(self, D: int, bn_decay: float, steps_per_day: int, use_bias: bool, device):
    ...
    def forward(self, SE: torch.FloatTensor, TE: torch.FloatTensor, T: int) -> torch.FloatTensor:
        ...
        SE = self._fully_connected_se(SE) # (1, 1, N, D)
        dayofweek = torch.empty(TE.shape[0], TE.shape[1], 7) #(B, 2T, 7)
        timeofday = torch.empty(TE.shape[0], TE.shape[1], T) #(B, 2T, 288)
        for i in range(TE.shape[0]): #(B, 2T, 2)
            dayofweek[i] = F.one_hot(TE[..., 1][i].to(torch.int64) % 7, 7)
        for j in range(TE.shape[0]):
            timeofday[j] = F.one_hot(TE[..., 0][j].to(torch.int64) % T, T)
        TE = torch.cat((dayofweek, timeofday), dim=-1) #(B, 2T, 7+288)
        TE = TE.unsqueeze(dim=2).to(self.device) #(B, 2T, 1, 7+288)
        TE = self._fully_connected_te(TE) # (B, 2T, 1, D)
        del dayofweek, timeofday
        return SE + TE # (B, 2T, N, D)
...
class TemporalAttention(nn.Module):
    ...
    def __init__(self, K: int, d: int, bn_decay: float, mask: bool=True):
        ...
        self._fully_connected = FullyConnected(input_dims=D, units=D, activations=F.relu,
                                               bn_decay=bn_decay)
    @profile(stream = fp)
    def forward(self, X: torch.FloatTensor, STE: torch.FloatTensor, device) -> torch.FloatTensor:
        """
        ...
        if self._mask:
            ...
            mask = mask.unsqueeze(0).unsqueeze(0).to(torch.bool).to(device)
            mask = mask.repeat(self._K * batch_size, num_nodes, 1, 1)
            # mask = mask.to(torch.bool).to(device)
            condition = torch.FloatTensor([-2 ** 15 + 1]).to(device)
...
class SpatioTemporalAttention(nn.Module):
    ...
    def __init__(self, K: int, d: int, bn_decay: float, mask: bool):
        ...
        self._temporal_attention = TemporalAttention(K, d, bn_decay, mask)
    ...
    def forward(self, X: torch.FloatTensor, STE: torch.FloatTensor, device) -> torch.FloatTensor:
        ...
        HT = self._temporal_attention(X, STE,device)
...
class GMAN(nn.Module):
    ...
    def __init__(self, args):
        ...
        d = args.gm_d
        D = K * d
        use_bias = args.gm_use_bias
        mask = args.mask
        bn_decay = args.bn_decay
        self.device = args.device
        self.SE = args.SE
        self.num_his = args.input_length
        self.steps_per_day = args.steps_per_day
        self._st_embedding = SpatioTemporalEmbedding(D, bn_decay, self.steps_per_day, use_bias,self.device)
    ...
    def forward(self, trainX: torch.FloatTensor, trainZ: torch.FloatTensor) -> torch.FloatTensor:
        """
