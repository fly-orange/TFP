...
class SpatioTemporalEmbedding(nn.Module):
    ...
    def forward(self, SE: torch.FloatTensor, TE: torch.FloatTensor, T: int) -> torch.FloatTensor:
        ...
        TE = TE.unsqueeze(dim=2).to(self.device)
...
class TemporalAttention(nn.Module):
    ...
    def forward(self, X: torch.FloatTensor, STE: torch.FloatTensor, device) -> torch.FloatTensor:
        ...
        if self._mask:
            ...
            condition = torch.FloatTensor([-2 **15+1]).cuda()
...
class SpatioTemporalAttention(nn.Module):
    ...
    def __init__(self, K: int, d: int, bn_decay: float):
        ...
        self._temporal_attention = TemporalAttention(K, d, bn_decay, mask=True)
...
class GMAN2(nn.Module):
    ...
    def __init__(self, args):
        ...
        mask = args.gm_mask
        bn_decay = args.gm_bn_decay
        self.device = args.device
        self.SE = args.SE
        self._num_his = args.input_length
        self._steps_per_day = steps_per_day
        self._st_embedding = SpatioTemporalEmbedding(D, bn_decay, steps_per_day, use_bias,self.device)
